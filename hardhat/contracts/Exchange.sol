...
contract Exchange is ERC20 {
    ...
    function addLiquidity(uint _amount) public payable returns (uint) {
        ...
        /**
         * If the reserve is empyt, intake any user supplied value for `Ether` and `Crypto Dev` tokens because there is no ratio currently
         */
        if (cryptoDevTokenReserve == 0) {
            // Transfer the `cryptoDevToken` from the user's account to the contract
            cryptoDevToken.transferFrom(msg.sender, address(this), _amount);
            // Take the current ethBalance and min `ethBalance` amount of LP tokens to the user
            // `liquidity` provided is equal to the `ethBalance` because this is the first time user is adding `Eth` to the contract, so whatever `Eth` contract has is equal to the one supplied by the user in the current `addLiquidity` call
            ...
            /**
             * If the reserve is not empyt, intake any user supplied value for the `Ether` and determine according to the ratio how many `Crypto Dev` tokens need to be supplied to prevent any large price impacts because of the additional liquidity
             */

            // Eth Reserve should be the current ethBalance subtracted by the value of Ether sent by the user in the current `addLiquidity` call

            uint ethReserve = ethBalance - msg.value;
            // Ratio should always be maintained so that there are no major price impacts when added liquidity
            ...
            // The amount of LP tokens that wuld be sent to the user should be proportional to the liquidity of ether added by the user
